...
def LagrangeBasisParamDervEvaluation(p,pts,xi,a):
    ...
    for j in range(len(pts)):
        ...
        basis_derivative += solution * (1 / (pts[a]-pts[j]))
...
def LagrangeBasisDervParamMultiD(A,degs,interp_pts,xis,dim):
    ...
    for i in range(len(degs)):
        ...
        lagrange *= MultiDimensionalBasisFunctions.LagrangeBasisEvaluation(degs[i],interp_pts[i],xis[i],idxs[i])
    
    derivative = LagrangeBasisParamDervEvaluation(degs[dim],interp_pts[dim],xis[dim],idxs[dim])
    
    output = lagrange * derivative
...
def PlotLagrangeBasisDerivatives(p,pts,n_samples = 101):
    ...
    for a in range(0,p+1):
        ...
        plt.plot(xis,vals)
    ax.grid(linestyle='--')
    plt.title("Derivatives of the Lagrange Basis Functions")
    plt.xlabel("Parent Domain")
    plt.ylabel("Spatial Domain")
    plt.legend(["Basis Function 1", "Basis Function 2", "Basis Function 3"])
    plt.show()
...
degs = [2,2]
pts = [np.linspace(-1,1,degs[i]+1) for i in range(len(degs))]
dim = 1
n = (degs[0]+1)
m = (degs[1]+1)

for A in range(n*m):
    PlotTwoDBasisFunctionParentDomain(A,degs,pts,dim)