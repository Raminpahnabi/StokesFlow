...
from LagrangePolys_OLD import LagrangeBasisEvaluation
...
def MultiDimensionalBasisFunction(A,degs,interp_pts,xis):
    """Evaluate the A-th (global, not multi-index) multi-dimensional
    Lagrange basis function, with degrees degs in each dimension,
    through a given list of points in each dimension, at the 
    multi-dimensional point xis.
    
    Args:
        A (int): The number of the polynomial to use
        p (int): Degree of polynomial
        pts (list[float]): A list of points to interpolate 
        xi (float): The point to evaluate the Lagrange basis function at 
        

    Returns:
        float: The numerical value of the basis function at that point
    """
