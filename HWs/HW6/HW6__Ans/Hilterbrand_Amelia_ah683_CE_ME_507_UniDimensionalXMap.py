...
from homework3 import LagrangeBasisEvaluation
from homework3 import PlotLagrangeBasisFunctions

# x(\xi) = \sum_{a=0}^p x_a * N_a(\xi)
def XMap(deg,spatial_pts,interp_pts,xi):
    sum = 0
    for a in range (deg + 1):
        Na = LagrangeBasisEvaluation(deg, interp_pts, xi, a)
        print(f"Basis function N_{a}({xi}) = {Na}")
        sum += spatial_pts[a] * Na
    return sum
...
PlotXMap(p,xa,Ea,101,False)
PlotLagrangeBasisFunctions(p,Ea)