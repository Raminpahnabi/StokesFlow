# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 15:47:56 2024
...
#Using the Lagrange function creates 1 basis function, will need to do it within a loop to get the sum
# x(\xi) = \sum_{a=0}^p x_a * N_a(\xi)
def XMap(deg,spatial_pts,interp_pts,xi):
    # complete this function
    total = 0
    for i in range(deg + 1):
        total += spatial_pts[i] * LagrangeBasisEvaluation(deg, interp_pts, xi, i)
    return total
...
#Establish values to input PlotXMap for each problem
deg = 1
spatial_pts = [0, 1]
interp_pts = [-1, 1]
