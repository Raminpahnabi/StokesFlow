...
from homework3 import InterpolateFunction
...
def MultiDimensionalBasisFunctionIdxs(idxs,degs,interp_pts,xis):
    ...
    for i in range(len(idxs)):
        value = value * LagrangeBasisEvaluation(degs[i],interp_pts[i],xis[i],idxs[i])
    return value
...
#px = 1
#py = 1
#for i in range((px+1) * (py+1)):
    #PlotTwoDimensionalParentBasisFunction(i,[px,py])
