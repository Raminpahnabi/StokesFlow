...
def XMap(deg,spatial_pts,interp_pts,xi):
    ...
    for a in range(0,deg+1):
        xe += spatial_pts[a]*lp.LagrangeBasisEvaluation(deg,interp_pts,xi,a)
    return xe
...
def PlotXMap(deg,spatial_pts,interp_pts, npts=101, contours=True):
    ...
    # evaluate as a contour plot
        ...
        fig, ax = plt.subplots()
        surf = ax.contourf(Z,Xi2,Xi,levels=100,cmap=matplotlib.colormaps['binary'])
        ax.set_xlabel(r"$x$")
        ax.yaxis.set_ticklabels([])
        fig.colorbar(surf)
        plt.show()
        
# PlotXMap(1,[0.0,1.0],[-1.0,1.0],contours=False)

# PlotXMap(2,[0.5,1.0,1.5],[-1.0,0,1.0])