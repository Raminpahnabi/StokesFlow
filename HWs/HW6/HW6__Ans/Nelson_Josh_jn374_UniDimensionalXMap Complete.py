...
def XMap(deg,spatial_pts,interp_pts,xi):
    ...
    for i in range(deg + 1):
        x += LagrangeBasisEvaluation(deg, interp_pts, xi, i) * spatial_pts[i]
        
    return x 

def PlotXMap(deg,spatial_pts,interp_pts, npts=101, contours=False):
...
# PlotXMap(1, [0,1], [-1,1])
# PlotXMap(2, [0.5,1,1.5], [-1,0,1])
# PlotXMap(2, [0.5,0.7,1.5], [-1,-0.6,1])
PlotXMap(2, [0.5,0.7,1.5], [-1,0,1])


"""
The mapping is linear when the spatial and interpretative proints match in spacing.
[0.5,0.7,1.5], [-1,0,1] The spacing here doesn't match so the mapping isn't linear.
"""